...
mu = [2; 5];                % Means
sigma = [0.3 0.5]; cat(3,[0.5],[0.3]); % Covariances
...
sKernelParams.sDistParams.u = 1;

beta = 2*sigma.^2/sKernelParams.omega^2;
...
for c = 1:nComponents
    for m = 0:nEigsSingleComponent-1
        sKernelParams.beta = beta(c);
        sKernelParams.sDistParams.mu_1D = mu(c);
        sKernelParams.sDistParams.sigma = sigma(c);
        tPhiAnalytic(c,:,m+1) = sqrt(nComponents/nPoints)*phi(sKernelParams, m, xPoints);
        mLambda(c,m+1) = lambda(sKernelParams,m);
    end
end
