...
%% Train
[ mPhiAnalyticTrain, vLambdaAnalytic ] = CalcAnalyticEigenfunctions(sSimParams, sKernelParams, sDataset.sData.x, b_normalize);

W = CalcAdjacency(sKernelParams, sDataset.sData.x);
D = diag(sum(W,1));
L = D - W;

[alpha, c] = eigrls(sDataset.sData.y, mPhiAnalyticTrain, diag(vLambdaAnalytic), gamma_A, gamma_I, L);
sClassifier.c = c;
sClassifier.vPhi_xTrain_c = mPhiAnalyticTrain*c;

%% Test
[ mPhiAnalyticTest, ~ ] = CalcAnalyticEigenfunctions(sSimParams, sKernelParams, sDataset.sData.xt, b_normalize);
...
tic;
[ mPhi_m_X, ~ ] = CalcAnalyticEigenfunctions(sSimParams, sKernelParams, X, b_normalize);
run_time = toc;
fprintf('Phi time = %f\n', run_time);
...
if sSimParams.b_plotEigenfunctions
    ...
    PlotEigenfuncvecScatter(sSimParams, 'Two_moons', X, [], firstEigenIdxToPlot, lastEigIdxToPlot, mPhi_m_X, vLambdaAnalytic, 'Analytic')
end
