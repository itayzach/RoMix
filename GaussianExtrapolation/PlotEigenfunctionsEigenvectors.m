...
if sParams.dim == 1
    ...
    saveas(fig,[sParams.sSim.outputFolder filesep 'fig_eigenvectors_1d'], 'epsc');
...
%     sgtitle('Kernel (analytic) Eigenfunctions')
    ...
    saveas(fig,[sParams.sSim.outputFolder filesep 'fig_pdf_2d'], 'epsc');
...
%                 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'Interpreter', 'latex', 'FontSize', 14)
    ...
    saveas(fig,[sParams.sSim.outputFolder filesep 'fig_eigenfunctions_2d'], 'epsc');
    ...
    if ~isempty(mPhi_A)
        ...
        set(gca,'FontSize', 14);
        set(gcf,'Position', [x0 y0 600 400])
        saveas(fig,[sParams.sSim.outputFolder filesep 'fig_histogram_2d'], 'epsc');
%                 hold on
%                 [ ~, maxIdx] = max(mHist3(:));
...
%             end
        ...
        set(gcf,'Position', [x0 y0 width height])
        saveas(fig,[sParams.sSim.outputFolder filesep 'fig_eigenvectors_2d'], 'epsc');
    end
else
