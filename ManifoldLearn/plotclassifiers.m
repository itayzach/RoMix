...
hold on;
title([method '  ('  kerneltype  ') : ' '\sigma =' num2str(kernelparam) ], 'FontSize', 20);
if size(lambda,2)==2
  xlabel([' \gamma_A = ' num2str(lambda(1)) '  \gamma_I = ' num2str(lambda(2))], 'FontSize', 20);
else
  xlabel([' \gamma_I = ' num2str(lambda(1))], 'FontSize', 20);  
end
